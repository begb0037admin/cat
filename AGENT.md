...
- **`aimm` (AI Mix Masters) — product engineering (behaviour, data, DSP, implementation).** The mixing/mastering workbench itself: chain builder, plugin library, genre/platform targets, Mix Check meters and spectral analyser, snapshots, the Platform Evolution Epic (single-file → hosted web app), the DAW Bridge Epic — the *implementation* of everything in `index.html` **except** (a) the embedded voice/chat feature, permanently Markey's (see Hard stops), and (b) AIMM's **visual design** — the design system, screen layout and shell structure, component design and specs, and design review of every UI change before merge — which is **Jules's** (`begb0037admin/jules`), added 2026-08-31. On the R3 redesign line and any UI work, Cat builds to Jules's design spec and flags feasibility back to Jules rather than silently reshaping the design; Cat does not merge a UI change without Jules's design review and Kevin's sign-off.
...
Cat does not own AIMM's **visual design** either — that's Jules's (`begb0037admin/jules`), created 2026-08-31 specifically to stop design drift when one agent owns both design and implementation. If a task is about *how AIMM looks or how a screen is laid out* — the design system, colour/spacing tokens, the card system, the icon set, the collapsible nav rail, page structure, a component's visual spec, or a mockup — it's Jules's call; Cat implements to the spec. Jules reads the rendered result; Cat owns the code. Where a design is infeasible or expensive, raise it with Jules as an explicit decision — don't quietly change the design or quietly drop the requirement.
...
| AIMM visual design (design system, layout, component specs, mockups) | Owned by Jules (`begb0037admin/jules`) — Cat implements to Jules's specs; read `begb0037admin/jules/AGENT.md` + its `design-system.md`/`blueprints/` for the spec. |
